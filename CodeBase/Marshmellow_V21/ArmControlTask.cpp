...
void ArmControlTask::enable() {
  if (!servoBig.attached())
    servoBig.attach(SERVO_ARM_PIN);
  if (!servoSmall1.attached())
    servoSmall1.attach(SERVO_GRIPPER_1_PIN);
  if (!servoSmall2.attached())
    servoSmall2.attach(SERVO_GRIPPER_2_PIN);
}

void ArmControlTask::disable() {
  if (servoBig.attached())
    servoBig.detach();
  if (servoSmall1.attached())
    servoSmall1.detach();
...
void ArmControlTask::startMove(ServoMotion &m, int target, int speed) {
  m.target = target;
  m.speed = speed;
  m.lastStep = millis();
  m.active = true;
...
void ArmControlTask::updateMove(ServoMotion &m) {
  if (!m.active)
    return;
...
void ArmControlTask::grabberAngle(int angle) {
...
void ArmControlTask::armAngle(int angle) {
...
void ArmControlTask::preparePickup() {
